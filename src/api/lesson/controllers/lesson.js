...
// const getCourseForLesson = async (strapi, lessonId) => {
...
// const studentCanViewCourse = async (strapi, userId, courseId) => {
...
//       const courseId = typeof courseFilter === 'object' ? courseFilter?.id ?? courseFilter?.$eq : courseFilter;
//       if (!courseId) {
...
module.exports = createCoreController('api::lesson.lesson', ({ strapi }) => ({
  ...
  async create(ctx) {
    ...
    // that's the format Strapi 5's core create action expects for relations.
