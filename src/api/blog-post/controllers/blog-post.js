...
module.exports = createCoreController('api::blog-post.blog-post', ({ strapi }) => ({
  ...
  // ctx.params.id is the post's documentId (Strapi 5 route convention).
  async findOne(ctx) {
    const user = ctx.state.user;
    const post = await strapi.db.query('api::blog-post.blog-post').findOne({
  ...
  async create(ctx) {
    ...
    // Relation values for the core create action need the author's documentId.
    const dbUser = await strapi.db.query('plugin::users-permissions.user').findOne({
      where: { id: user.id },
    });
    ctx.request.body.data = ctx.request.body.data || {};
    ctx.request.body.data.author = dbUser.documentId;
    return super.create(ctx);
  },
